"""Polygon (MATIC) whale collector using PolygonScan API."""
import logging
...
class MaticCollector(BaseCollector):
    chain = "MATIC"
    native_symbol = "MATIC"
    ...
    async def poll(self, db: AsyncSession) -> None:
        self.log_poll_start()
        api_key = self._settings.polygonscan_api_key
        if not api_key:
            logger.warning("[MATIC] No POLYGONSCAN_API_KEY set, skipping")
            return
        ...
        try:
            ...
            start_block = max(last_block + 1, latest_block - 30)  # Polygon ~2s blocks
            ...
            logger.error("[MATIC] Poll error: %s", exc)
    ...
    async def _get_latest_block(self, api_key: str) -> int:
        async with httpx.AsyncClient(timeout=10) as client:
            resp = await client.get(POLYGONSCAN_URL, params={
